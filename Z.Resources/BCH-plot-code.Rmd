...
m_step2  <- mplusObject(
  ...
  VARIABLE = 
 "usevar = BCHW1-BCHW3 read_tes math_tes;
  missing are all (999); 
  classes = c(3); 
...
## EXAMPLE 2: Harrassment & Discipline LCA indicators (4-class solution)
...
"math_test","Average math test assessment score at school" ) %>% 
gt() %>% 
tab_header(title = md("**LCA Indicators & Auxiliary Variables: Math Attitudes Example**"),
           subtitle = md("&nbsp;")) %>%
tab_row_group(group = "LCA Indicator Variables", rows = 1:6) %>% 
tab_row_group(group = "Auxiliary Variables", rows = 7:9) %>%
row_group_order(groups = c("LCA Indicator Variables","Auxiliary Variables")) %>% 
...
## 1.8 Distal Outcome Plot (example 2)

# --------------------------------------------------------------------------------------

This syntax reads in the `step2` model & extract model parameter estimates.
```{r, eval=TRUE}
model_step2 <- readModels(here("bch_ex2_mplus", "step2_bch_ex2.out"), quiet = TRUE)
  
model_step2 <- data.frame(model_step2$parameters$unstandardized)
...
distal_data <- model_step2 %>% 
  filter(paramHeader == "Means") %>% 
  mutate(param = case_when(
      param == "READ_TES" ~ "Reading Test",
      param == "MATH_TES" ~ "Math Test")) %>% 
  mutate(LatentClass = factor(LatentClass,
      labels = c("High/moderate on disab., race, & sex.",
                 "Low on disab. & high/moderate on race, & sex.",
                 "Low on disab., race & high on sex.",
                 "Low on disab., race, & sex."))) %>% 
...
ggplot(distal_data, 
  ...
  theme_cowplot() +
  labs(x = "", y = "Mean Value", fill = "Latent Class: Harass. & Discip. reported...") +
...
```{r}
ggsave(here("figures","Distal_barplot_ex2.png"), dpi=300, height=3, width=6, units="in")  
```
